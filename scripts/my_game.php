  ...
  <div class='feed-list'>
    <section class="tile color transparent-black">
      ...
      <div class="tile-header">
        <h1><strong>Best</strong> Rounds</h1>
      </div>
      ...
      <div class="tile-body">
        <table class='table table-striped'>
          ...
          <tbody>
            <?php 
            $get_best_scores = "SELECT * FROM user_rounds WHERE user_id = '".$row_user['user_id']."' ORDER BY total ASC LIMIT 5"; 
            $run_best_scores = mysqli_query($con, $get_best_scores); 
            while($row_best_scores = mysqli_fetch_array($run_best_scores)) {
                echo "
                <tr>
                    <td>".date("M d, Y", strtotime($row_best_scores['date_played']))."</td>
                    <td><a href='round-details.php?round_id=".$row_best_scores['round_id']."' style='color:#fff;'> ".$row_best_scores['course_name']." </a></td>
                    <td> ".$row_best_scores['total']." </td>
                </tr>";
            }
            ?>
          </tbody>
        ...
        <div class='tile-header'>
          <h1><strong>All</strong> Rounds</h1>
        </div>
...
<div class="row"> 
  <div class='feed-list'>
    <section class="tile color transparent-black">
      <!-- tile header -->
      <div class="tile-header">
        <h1><strong>Quick</strong> Stats</h1>
      </div>
      <!-- /tile header -->
      <!-- tile body -->
      <div class="tile-body">
        ...
        echo"
          ...
          <hr style='width:80%; margin:0 auto; margin-bottom:15px;'>
          <span style='font-size:16px; color:#ececec;'><strong>Fairway </strong>/ GIR %</span>";
          if(isset($_GET['user_id'])) {
            global $con; 
            $user = $_GET['user_id']; 
            $get_player = "SELECT * 
          ...
          echo "
            <table class='table table-striped' style='margin-top:15px; margin-bottom:25px;'>
              <thead>
                <tr>
                  <th style='width:33%; text-align:center;'><span id='topTooltip' data-toggle='tooltip' data-placement='top' title='' data-original-title='Hit Fairway Percentage'>Fairway %</span></th>
                  <th style='width:33%; text-align:center;'><span id='topTooltip' data-toggle='tooltip' data-placement='top' title='' data-original-title='Green In Regulation Percentage'>GiR %</span></th>
                  <th style='width:33%; text-align:center;'><span id='topTooltip' data-toggle='tooltip' data-placement='top' title='' data-original-title='Par or Better When Hit on Fairway'>Par+ %</span></th>
                </tr>
              </thead>
              <tbody>
                <tr>
